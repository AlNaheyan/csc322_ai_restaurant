...
class CustomerService {
  ...
  async addDeposit(userId, amount) {
    const minDeposit = parseFloat(process.env.MIN_DEPOSIT) || 10.0;
    ...
    try {
      ...
      await customer.update({ deposit_balance: balanceAfter }, { transaction });
  ...
  async checkVipEligibility(customerId) {
    ...
    const spendingThreshold =
      parseFloat(process.env.VIP_SPENDING_THRESHOLD) || 100.0;
    ...
    const meetsOrders =
      customer.order_count >= orderThreshold &&
      customer.User.warning_count === 0;
  ...
  async requestAccountClosure(userId, reason) {
    ...
    if (parseFloat(customer.deposit_balance) > 0) {
      throw new Error(
        "Cannot close account with remaining balance. Please withdraw or use your balance first."
      );
    }
    ...
    try {
      ...
      return {
        message:
          "Account closure request submitted successfully. Your account has been deactivated.",
      };
    } catch (error) {
      await transaction.rollback();
      throw error;
  ...
  async getPendingRegistrations() {
    const pendingCustomers = await Customer.findAll({
  ...
  async approveRegistration(customerId) {
    const customer = await Customer.findByPk(customerId, {
      include: [{ model: User }],
    });
  ...
  async rejectRegistration(customerId, reason) {
